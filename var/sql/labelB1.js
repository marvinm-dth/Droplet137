...
const merge                 = require('lodash.merge');          // deep merge
...
let elementsConfig = [
  {name:'sidebar', type:'text', text:'DTH ITEM', weight:'700', fontSize:0.2, fontColor:'black', fillColor:'white', bounds:{width:0.5,height:1}, position:{x:0,y:1}, rotation:270 },
  {name:'item_name',type:'text',table:'home_depot_items',lookupColumn:'material_id',returnColumn:'item_desc',truncate:20,weight:'700',fontSize:0.14,fontColor:'black',bounds:{width:0.55,height:0.3},position:{x:0.15,y:0.05}},
  {name:'order_id', type:'text',table:'home_depot_order_history',lookupColumn:'order_id',returnColumn:'order_id',weight:'700',fontSize:0.14,fontColor:'black',bounds:{width:0.55,height:0.12},position:{x:0.15,y:0.5}},
  {name:'sku',      type:'text',table:'home_depot_items',lookupColumn:'material_id',returnColumn:'internal_sku',fallbackColumn:'temp_internal_sku',generateFallback:true,weight:'700',fontSize:0.13,fontColor:'black',bounds:{width:0.6,height:0.11},position:{x:0.15,y:0.65}},
  {name:'location', type:'text',text:'Bay 1 | Shelf 4 | Bin 18',weight:'700',fontSize:0.11,fontColor:'black',bounds:{width:0.55,height:0.12},position:{x:0.15,y:0.85}},
  {name:'qr_code',  type:'qr', sourceElement:'sku',bounds:{width:0.4,height:0.8},position:{x:0.60,y:0}},
  {name:'divider',  type:'line',orientation:'vertical',position:{x:0.11,y:0.05},length:0.94,thickness:4,color:'black',style:'solid'}
];
 ...
 * Returns an array of lines; drawing is the caller’s job.
 */
function wrapLines(ctx, text, maxWidth) {          // ⟵ NEW
  if (!text) return [''];
  ...
  for(const el of cfg){
    ...
    switch(el.type){
      case 'text': {
        ...
        // word-wrap ─ choose to wrap unless el.wrap === false
        const doWrap   = el.wrap !== false;                        // ⟵ NEW
        const lines    = doWrap ? wrapLines(ctx, content, w) : [content];
        const lh       = pxSize * 1.2;                             // line-height
        for (let i = 0; i < lines.length; i++) {                   // ⟵ CHANGED
          const ly = y + i * lh;
          if (ly + lh > y + h) break;       // stop if exceeds height
          ctx.fillText(lines[i], x, ly, w);
        }
...
async function createPdf(bufs, orderId){
  ...
  bufs.forEach((b,i)=>{
    ...
    doc.image(b, 0, 0, { width: PT_W, height: PT_H }); // scale to page
  });
...
app.patch('/elements-config/:name', (req,res)=>{
  ...
  elementsConfig[idx] = merge({}, elementsConfig[idx], req.body); // deep merge
