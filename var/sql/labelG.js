...
// Default size in points: 2"x1" = 144x72 points (1 inch = 72 points)
const labelDimensions = {
  "2x1": [144, 72],
};

async function createPdf(order, labelSize = "2x1") {
  if (!order.internet_sku_number || !order.order_id)
    throw new Error("Missing necessary order details");

  const dimensions = labelDimensions[labelSize] || labelDimensions["2x1"];
  ...
  const doc = new PDFDocument({ size: dimensions });
  doc.pipe(fs.createWriteStream(filePath));

  doc.image(qrBuffer, 10, 10, { width: dimensions[0] - 20 });
...
async function pollPendingOrders() {
  try {
    ...
    for (const order of orders) {
      if (!order.internet_sku_number || !order.order_id) {
        console.warn(`Skipping order due to missing fields: ${order.order_id}`);
        continue;
      }
