...
# data = pd.read_csv("home_depot.csv")

# for i, row in data.iterrows():
#     db.update("home_depot_items", "material_id", row['material_id'], {"upc": row['upc']})
...
def add_order():
    ...
    if home_depot_items.data:
        ...
        return render_template("add_order.html", home_depot_items=home_depot_data, supplier_data=supplier_data, user=current_user, approver_data=approver_data, request_materials=request_materials, user_order_materials=user_order_materials, pending_request_materials=pending_request_materials)
        ...
        for supplier in suppliers:
            ...
            for id, name in zip(data['approver_ids'], data['approver_names']):
                notif_data = {'category': "order",
                            "url": f"https://137.184.148.164:5023/approval_order/{order_insert.data[0]['order_id']}",
...
def submit_approval_order(order_id):
    ...
    if approval_status == "Approved":
        ...
        db.insert("dth_notification", {'url': f"https://137.184.148.164:5023/approval_order/{order_id}",
...
def submit_supplier():
    ...
    while f"materials[{index}][material_id]" in request.form:
        ...
        db.insert("home_depot_items", {"internet_sku_number": supplier_material_id,
                                       ...
                                       "item_image": f"https://137.184.148.164:5023/image/{str(supplier_name)}_{str(supplier_id)}.jpg"
                                       })
...
def submit_material(supplier_id):
    ...
    if response:
        ...
        db.insert("home_depot_items", {"internet_sku_number": supplier_sku_number,
                                       ...
                                       "supplier_id": supplier_id,
                                       "item_image": f"https://137.184.148.164:5023/image/{str(supplier_name)}_{str(supplier_sku_number)}.jpg"
                                       })
        ...
        for file in new_image:
            if file:
                ...
                db.insert("dth_delivery_images", {"delivery_id": delivery_data['delivery_id'],
                                                "image_url":f"https://137.184.148.164:5023/delivery_file/{filename}"})
                
        db.insert("dth_notification", {"url": f"https://137.184.148.164:5023/approval_delivery/{delivery_data['delivery_id']}",
                                       "requester_name": current_user.name,
                                       'requester_id': current_user.id,
                                       "approval_status": "pending",
                                       "approver_name": approver_name,
...
def inventory_onhand():
    ...
    if home_depot_items.data:
        home_depot_data = combined_items
        supplier_data = supplier_res.data
        return render_template("inventory_onhand.html", home_depot_items=home_depot_data, supplier_data=supplier_data, user=current_user)
...
def get_home_depot_items():
    ...
    for item in response.data:
        ...
        # Add supplier_name to the item dictionary
        item['supplier_name'] = supplier_name
        item['qr_info'] = f"https://137.184.148.164:5023/home_depot_item/{str(item['material_id'])}"

        # CHANGE THIS
        item['qr_inventory'] = f"https://137.184.148.164:5023/inventory/{str(item['material_id'])}"
    ...
    if response.data:
        ...
        supplier_data = supplier_res.data
        return render_template("home_depot_items.html", home_depot_data=home_depot_data, supplier_data=supplier_data, user=current_user)
    return jsonify({"message": "Checklist item not found"}), 404
...
def get_home_depot_order(order_id):
    ...
    if response_order.data:
        ...
        return render_template("order_number.html", order_data=order_data, order_history_data=order_history_data, other_files=other_files, approver_data=approver_data)
    return jsonify({"message": "Checklist item not found"}), 404
...
def update_order(order_id):
    ...
    if response_order.data:
        ...
        other_files = response_other_file.data
        print(other_files)
        return render_template("update_order.html", order_data=order_data, order_history_data=order_history_data, other_files=other_files)
...
def bulk_update():
    ...
    if response.data:
        ...
        return render_template("update_bulk_material.html",home_depot_items=combined_items, user=current_user, supplier_data=supplier_data)
    return jsonify({"message": "No data provided"}), 400
...
def bulk_upc_update():
    ...
    if response.data:
        ...
        supplier_data = response_supp.data
        return render_template("update_bulk_upc.html",home_depot_items=combined_items, user=current_user, supplier_data=supplier_data)
    return jsonify({"message": "No data provided"}), 400
...
def update_order_form(order_id):
    ...
    if (receipt_file) and (f"https://137.184.148.164:5023/receipt/receipt_{str(order_id)}.pdf" != order_data_old['supplier_receipt']):
        ...
        db.update("home_depot_orders", "order_id", order_id, {"supplier_receipt": f"https://137.184.148.164:5023/receipt/{receipt_filename}"})
    else:
        print("NO")
    ...
    if (qty_file) and (f"https://137.184.148.164:5023/order_file/order_{str(order_id)}.pdf" != order_data_old['supplier_order_file']):
        qty_filename = f"order_{order_id}.pdf"
        qty_file.save(os.path.join(app.config['UPLOAD_FOLDER_ORDER_FILE'], qty_filename))
        db.update("home_depot_orders", "order_id", order_id, {"supplier_order_file": f"https://137.184.148.164:5023/order_file/{qty_filename}"})
    ...
    if other_files:
        for file in other_files:
            ...
            other_file_dict = {"order_id": order_id,
                               "file_name": file_name,
                               "file_url": f"https://137.184.148.164:5023/order_file/{file_name}"}
...
def submit_update_material(material_id):
    ...
    if new_image:
        ...
        db.update("home_depot_items", "material_id", material_id, {"item_image": f"https://137.184.148.164:5023/image/{str(supplier_name)}_{str(new_supplier_sku)}.jpg"})
    ...
    for file in uploaded_files:
        if file:
            ...
            db.insert("dth_material_images", {"material_id": material_id,
                                              "image_url":f"https://137.184.148.164:5023/image/{filename}"})
...
def send_receive_material():
    ...
    for image in global_images:
        ...
        to_insert_img_data = {"receive_id" : receive_data.data[0]['id'],
                              "image": f"https://137.184.148.164:5023/receive/{filename}"}
    ...
    for material in receive_materials:
        ...
        for receive_image in receive_materials_images:
            ...
            to_insert_material_img = {"receive_item_id": receive_material_data.data[0]['id'],
                                      "image": f"https://137.184.148.164:5023/receive_item/{filename}"}
...
app.config['JWT_SECRET_KEY'] = '12345678'   # use a real secret in prod!
jwt = JWTManager(app)

# now import and mount the API blueprint
from api import api as api_bp
app.register_blueprint(api_bp)

# Attach it so that Blueprints can see it via current_app
app.db = db

# 2) now DEBUG: list routes (you should see /api/v1/login etc)
print("\n=== Registered Routes ===")
