...
// Read PDF, base64-encode & POST to bridge
async function sendToPrinter(pdfPath) {
  ...
  try {
    ...
    if (!res.ok) console.error("Print bridge error:", await res.text());
  } catch (e) {
    console.error("Failed to send to printer:", e);
  }
}
...
async function pollPendingOrders() {
  ...
  for (const order of data) {
    try {
      ...
      console.error(`Error for order ${order.order_id}:`, err);
    }
  }
}
...
app.listen(port, () =>
  console.log(`PDF & print bridge client listening on port ${port}`)
);
