...
// /* ── Health-check ─────────────────────────────────────────── */
// app.get("/", (_req, res) => res.send("Inventory API up"));

// /* ── Start server ─────────────────────────────────────────── */
// app.listen(PORT, () => console.log(`🚀 HTTP server listening on port ${PORT}`));


// server.js – Inventory API (with Mandarin support)
require("dotenv").config();
...
/* ── ITEMS ENDPOINTS (Mandarin-ready) ─────────────────────── */
...
/**
 * GET /items/:sku
 *   Fetch by internal_sku or temp_internal_sku (DTH‑prefixed) including Mandarin fields.
 */
app.get("/items/:sku", async (req, res) => {
  const { sku } = req.params;
  console.log(`→ GET /items/${sku}`);
  ...
  try {
    const { data, error } = await sb
      .from("home_depot_items")
      .select(
        "item_desc, item_desc_mandarin, item_image, internal_sku, temp_internal_sku"
      )
      .or(`internal_sku.eq.${sku},temp_internal_sku.eq.${sku}`)
      .maybeSingle();
    ...
    const skuUsed = data.internal_sku === sku ? data.internal_sku : data.temp_internal_sku;

    return res.json({
      item_name:      data.item_desc,
      item_name_cn:   data.item_desc_mandarin,   // NEW
      item_image:     data.item_image,
      sku_used:       skuUsed,
    });
  } catch (err) {
    console.error("! /items handler:", err.message);
    res.status(500).json({ error: err.message });
  }
});
...
/**
 * GET /items/backup/:backupSku
 *   Resolve backup_sku → internal_sku → item (incl. Mandarin fields).
 */
app.get("/items/backup/:backupSku", async (req, res) => {
  const { backupSku } = req.params;
  console.log(`→ GET /items/backup/${backupSku}`);
  ...
  try {
    ...
    // 2) home_depot_items lookup (Mandarin)
    const { data: item, error: itemErr } = await sb
      .from("home_depot_items")
      .select(
        "item_desc, item_desc_mandarin, item_image, internal_sku, temp_internal_sku"
    ...
    const skuUsed = item.internal_sku === resolvedSku ? item.internal_sku : item.temp_internal_sku;

    return res.json({
      item_name:      item.item_desc,
      item_name_cn:   item.item_desc_mandarin,  // NEW
      item_image:     item.item_image,
      sku_used:       skuUsed,
    });
  } catch (err) {
    console.error("! /items/backup handler:", err.message);
    res.status(500).json({ error: err.message });
  }
});

/* ── TRACKING (UUID + backupSku) ──────────────────────────── */
app.post("/track/:uuid/:action(received|missing)", async (req, res) => {
  const { uuid, action } = req.params;
  const payload = action === "received"
    ? { status: "Received", received_at: new Date().toISOString() }
...
/* ── ORDER-HISTORY CRUD (delivery_id is an int[] column) ─── */
app.post("/order-history", async (req, res) => {
  if (req.body.delivery_id && !Array.isArray(req.body.delivery_id)) {
    req.body.delivery_id = arrify(req.body.delivery_id);
  }
