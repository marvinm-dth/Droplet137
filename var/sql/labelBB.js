...
// ─── Tracking-table helper ────────────────────────────────────────────────
async function insertTrackingRows(order){
  // fetch item metadata once
  const { data:item, error:itemErr } = await supabase
        .from('home_depot_items')
        .select('internal_sku,temp_internal_sku,external_sku,item_desc,item_name')
        .eq('material_id', order.material_id)
        .single();
  if(itemErr){
    console.error('Item lookup failed', itemErr);
    return;
  }

  const internalSku = item?.internal_sku || item?.temp_internal_sku || null;

  // build N rows (one per physical label)
  const rows = Array.from({length: order.order_qty_requested}, () => ({
    ...
    UUID         : crypto.randomBytes(8).toString('hex')   // 16-char ID
  }));
...
async function createPdf(bufs,id){
  ...
  const st=fs.createWriteStream(fp);
  doc.pipe(st);
...
async function printBuffers(bufs){
  ...
  for(const b of bufs){
    const res=await fetch(ep,{
      ...
      body:JSON.stringify({
        imageData:`data:image/png;base64,${b.toString('base64')}`,
        cut:true
      })
    });
...
app.put('/elements-config',(q,r)=>{
  if(!Array.isArray(q.body))
    return r.status(400).json({error:'Body must be array'});
  elementsConfig=q.body;
  r.json({ok:true,count:elementsConfig.length});
});
app.patch('/elements-config/:name',(q,r)=>{
  ...
  elementsConfig[i]=merge({},elementsConfig[i],q.body);
  r.json(elementsConfig[i]);
});
app.get('/last-order',(_,r)=>
  lastOrder ? r.json(lastOrder)
            : r.status(404).json({error:'No order yet'}));
app.post('/preview',async(q,r)=>{
  const o=q.body;
  if(!o?.order_id||!o?.material_id||!o?.order_qty_requested)
    return r.status(400).json({
      error:'order_id, material_id, order_qty_requested required'
    });
  try{
    const bufs=await createImageBuffers(o);
    ...
    r.status(500).json({error:String(err)});
  }
});
