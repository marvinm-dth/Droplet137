...
// Use the legacy PDF.js entrypoints:
...
// Fetch dynamic element data
async function fetchElementData(el, lookupVal) {
  if (!el.table || !el.returnColumn) return el.text || "";
  const { data, error } = await supabase
    .from(el.table)
    .select(el.returnColumn)
    .eq(el.lookupColumn, lookupVal)
    .single();
...
// Draw all elements onto one PDF page
...
// Convert each PDF page to PNG buffer with PDF.js + node-canvas
async function pdfToPNGBuffers(pdfPath) {
  const raw    = new Uint8Array(fs.readFileSync(pdfPath));
  const task   = pdfjsLib.getDocument({ data: raw });
  ...
  for (let p=1; p<=pdf.numPages; p++) {
    ...
    const cvs  = createCanvas(vp.width, vp.height);
    const ctx  = cvs.getContext("2d");
    await page.render({ canvasContext:ctx, viewport:vp }).promise;
    buffers.push(cvs.toBuffer("image/png"));
  }

  return buffers;
}

// Send each PNG to your printer bridge
async function sendToPrinter(pdfPath) {
  const pages = await pdfToPNGBuffers(pdfPath);
...
// Poll for new orders, generate PDF, update Supabase, then print
