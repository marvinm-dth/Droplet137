...
const masterElements = [
  ...
  {name:'sku-1', type:'text', text:'',   // will hold backup_sku
   weight:'700', fontSize:0.09, fontColor:'black',
   bounds:{width:0.60,height:0.11}, position:{x:0.7,y:0.70}, wrap:false},
  ...
  {name:'qr_code', type:'qr', text:'',   // will hold sku+uuid
...
/* ── Runtime state ─────────────────────────────────────────────────────── */
let lastLiveOrder   = null;   // last DB-processed order
let previewOrder    = null;   // last preview request
let previewBuffers  = [];
  ...
  for(const el of cfg){
    ...
    switch(el.type){
      case 'text': {
        ...
        let px=el.fontSize*PX_H, lines, lh;
        for(let tries=0; tries<40 && px>6; tries++){
          meas.font=mkFont(px);
        ...
        ctx.save();
        ctx.font=mkFont(px); ctx.fillStyle=el.fontColor||'black';
      ...
      case 'line': {
        ctx.save(); ctx.strokeStyle=el.color||'black'; ctx.lineWidth=el.thickness||1;
        ctx.beginPath();
...
/* Make a label; optionally skip DB insert */
async function buildLabel({order, internalSku, externalSku, itemDesc, preview=false}){
  ...
  if(!preview){
    ...
    const {error}=await supabase.from('all_items_tracking').insert(row);
...
async function buildLabelsForOrder(order){
  ...
  for(let i=0;i<order.order_qty_requested;i++){
    bufs.push(await buildLabel({order,internalSku,externalSku,itemDesc}));
  }
  return bufs;
}

/* Build preview buffers (read-only) */
async function buildPreviewBuffers(order){
  const {data:item,error}=await supabase
     .from('home_depot_items')
     .select('internal_sku,temp_internal_sku,sku_number,internet_sku_number,item_desc')
     .eq('material_id',order.material_id)
     .single();
  if(error) throw new Error('Item lookup failed for preview');
  ...
  const bufs=[];
  for(let i=0;i<order.order_qty_requested;i++){
    bufs.push(await buildLabel({
      order, internalSku, externalSku, itemDesc, preview:true
    }));
  }
  return bufs;
}

/* Package to PDF (95 % scale) */
async function createPdf(bufs,id){
  const fp=path.join('pdf',`${id}.pdf`); ensureDir(fp);
  const SCALE=0.95; const newW=PT_W*SCALE, newH=PT_H*SCALE;
...
async function poll(){
  ...
  for(const o of orders){
    try{
      ...
      lastLiveOrder=o;                                    // save for /last-order
...
app.post('/preview',async(req,res)=>{
  ...
  try{
    const bufs=await buildPreviewBuffers(o);
    const pdf =await createPdf(bufs,`${o.order_id}_preview`);
    previewOrder=o; previewBuffers=bufs; previewPdfPath=pdf;
    res.json({pages:bufs.length,pdf:'/preview/pdf',png:'/preview/image/0'});
  }catch(err){ res.status(500).json({error:String(err)}); }
});
app.get('/preview/image/:i?',(req,res)=>{
  const idx=Number(req.params.i||0);
  if(!previewBuffers[idx]) return res.status(404).send('No preview');
  res.type('png').send(previewBuffers[idx]);
});
app.get('/preview/pdf',(_,res)=>{
  if(!previewPdfPath||!fs.existsSync(previewPdfPath))
    return res.status(404).send('No PDF');
  res.download(previewPdfPath);
});

/* ── Last-order endpoint (HTML viewer needs this) ───────────────────────── */
app.get('/last-order',(_,res)=>{
  if(previewOrder)      return res.json(previewOrder);
  if(lastLiveOrder)     return res.json(lastLiveOrder);
  return res.status(404).json({error:'No order yet'});
});
...
app.put('/elements-config',(q,r)=>{
  ...
  masterElements.length=0; masterElements.push(...q.body); r.json({ok:true,count:masterElements.length});
});
app.patch('/elements-config/:name',(q,r)=>{
  ...
  masterElements[i]=merge({},masterElements[i],q.body); r.json(masterElements[i]);
});
