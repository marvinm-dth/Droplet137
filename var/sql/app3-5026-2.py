...
def build_label_pdf(order_id: int, line_items: list[dict]) -> str:
    """
    • Inserts 1 row per physical item into all_items_tracking
    • Renders one label per page into /var/sql/dth_materials/labels/order_<id>.pdf
    • Returns the public URL to that PDF
    """
    pdf_dir = "/var/sql/dth_materials/labels"
    os.makedirs(pdf_dir, exist_ok=True)
    pdf_path = os.path.join(pdf_dir, f"order_{order_id}.pdf")
    pdf_url  = f"https://app.137.184.148.164.nip.io/order_file/labels/order_{order_id}.pdf"

    buf = BytesIO()
    c   = canvas.Canvas(buf)

    for line in line_items:                                # one row per SKU
        qty = int(line["order_qty_requested"])
        for _ in range(qty):                               # one row per piece
            uid = uuid.uuid4()

            # 1️⃣  insert serialized row
            db.insert("all_items_tracking", {
                "external_sku": line["internet_sku_number"],
                "internal_sku": line["material_id"],
                "item_name":    line["item_desc"],
                "item_desc":    line["item_desc_mandarin"],
                "label_size":   line.get("label_size"),
                "order_number": line["order_item_number"],   # FK-free
                "ordered_at":   datetime.utcnow(),
                "status":       "pending",
                "UUID":         str(uid)
            })

            # 2️⃣  draw a label (adjust coords for your sizes)
            qr = qrcode.make(str(uid))
            qr_buf = BytesIO(); qr.save(qr_buf, "PNG")
            c.drawImage(ImageReader(qr_buf), 20, 20, 80, 80)
            c.setFont("Helvetica", 7)
            c.drawString(110, 90, f"SKU: {line['internet_sku_number']}")
            c.drawString(110, 76, f"Name: {line['item_desc'][:24]}")
            c.drawString(110, 62, f"Order: {order_id}")
            c.drawString(110, 48, datetime.utcnow().strftime("%Y-%m-%d"))
            c.showPage()
    ...
    return pdf_url
# +++ NEW +++
def generate_labels_pdf(order_id: int, sku_lines: list[dict]) -> str:
    """
    Creates <labels>/order_<order_id>.pdf and returns the public URL.
    One page = one physical item label.
    """
    pdf_dir = '/var/sql/dth_materials/labels'
    os.makedirs(pdf_dir, exist_ok=True)
    path = os.path.join(pdf_dir, f'order_{order_id}.pdf')
    ...
    for line in sku_lines:
        qty = int(line['order_qty_requested'])
        for _ in range(qty):
            # 1️⃣  insert one row into all_items_tracking
            new_uuid = uuid.uuid4()
            db.insert('all_items_tracking', {
                'external_sku': line['internet_sku_number'],
                'internal_sku': line['material_id'],
                'item_name':    line['item_desc'],
                'item_desc':    line['item_desc_mandarin'],
                'label_size':   line.get('label_size'),          # text
                'order_number': line['order_item_number'],       # FK-less
                'ordered_at':   datetime.utcnow(),
                'status':       'pending',
                'UUID':         str(new_uuid)
            })
            ...
            # 2️⃣  draw the label
            qr_img = qrcode.make(str(new_uuid))
            qr_buf = BytesIO(); qr_img.save(qr_buf, format='PNG')
            c.drawImage(ImageReader(qr_buf), 20, 20, 80, 80)
            c.setFont('Helvetica', 7)
            c.drawString(110, 90, f'SKU: {line["internet_sku_number"]}')
            c.drawString(110, 78, f'Name: {line["item_desc"][:22]}')
            c.drawString(110, 66, f'Order: {order_id}')
            c.drawString(110, 54, datetime.utcnow().strftime('%Y-%m-%d'))
            c.showPage()
    ...
    return f'https://app.137.184.148.164.nip.io/order_file/labels/order_{order_id}.pdf'
...
def submit_order():
    ...
    for supplier in suppliers:
        ...
        order_insert = db.insert("home_depot_orders", header)
        order_id     = order_insert.data[0]['order_id']

        # ────────────────── 2.  APPROVERS  ───────────────
        for approver_id, approver_name in zip(data['approver_ids'],
                                              data['approver_names']):
            url = f"https://app.137.184.148.164.nip.io/approval_order/{order_id}"

            db.insert("dth_notification", {
                "category": "order",
                "url": url,
        ...
        # ────────────────── 3.  LINE ITEMS  ──────────────
        keywords_txt = ""
        lines_for_pdf = []               # collect safe rows for the PDF

        for line in supplier['items']:
            # pull static SKU data once
            sku = db.fetch_one("home_depot_items",
                               "material_id", line['material_id']).data[0]

            # mark request as added, if applicable
            if line.get("request_material_id"):
                db.update("dth_material_request", "id",
                          line["request_material_id"],
                          {"is_added_to_order": True})

            # enrich & clean the line dict
            line.update(order_insert.data[0])           # merge header cols
            line.update({
                "item_desc":          sku['item_desc'],
                "item_desc_mandarin": sku['item_desc_mandarin'],
                "label_size":         sku.get('label_size')   # NEW FIELD
            })
            for k in ("request_material_id","supplier_name","supplier_id",
                      "total_amount","supplier_order_file","supplier_receipt",
                      "notes","order_status","material_keywords",
                      "label_pdf_url"):
                line.pop(k, None)

            # insert & capture order_item_number
            inserted = db.insert("home_depot_order_history", line)
            line['order_item_number'] = inserted.data[0]['order_item_number']
            lines_for_pdf.append(line)                 # save for PDF

            keywords_txt += f"{sku.get('keywords','')},{sku.get('keywords_ch','')},{sku['item_desc']},{sku['item_desc_mandarin']},"

        db.update("home_depot_orders", "order_id", order_id,
                  {"material_keywords": keywords_txt})

        # ────────────────── 4.  LABEL PDF  ───────────────
        label_url = generate_labels_pdf(order_id, lines_for_pdf)
        db.update("home_depot_orders", "order_id", order_id,
...
# +++ NEW +++
@app.route('/api/scan_serial/<uuid:item_uuid>', methods=['POST'])
@login_required
def scan_serial(item_uuid):
    rec = db.fetch_one('all_items_tracking', '"UUID"', str(item_uuid))
    if not rec.data:
        return jsonify({'error': 'Label not found'}), 404
    row = rec.data[0]
    if row['status'] == 'received':
        return jsonify({'message': 'Already received'}), 200

    db.update('all_items_tracking', '"UUID"', str(item_uuid),
              {'status': 'received', 'received_at': datetime.utcnow()})

    # update aggregates in order history
    oid = row['order_number']
    line = db.fetch_one('home_depot_order_history',
                        'order_item_number', oid).data[0]
    db.update('home_depot_order_history', 'order_item_number', oid, {
        'order_qty_received': int(line['order_qty_received']) + 1,
        'order_qty_remaining': int(line['order_qty_remaining']) - 1
    })
    return jsonify({'ok': True})

@app.route('/api/flag_missing', methods=['POST'])
@login_required
def flag_missing():
    uid = request.json['item_uuid']
    db.update('all_items_tracking', '"UUID"', uid,
              {'status': 'missing', 'missing_flagged_at': datetime.utcnow()})
    return jsonify({'ok': True})
# +++ END NEW +++
...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=5023)#, ssl_context=('app3-5026-cert/cert.pem', 'app3-5026-cert/key.pem'))