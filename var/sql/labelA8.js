...
// Your element definitions, including the divider line
...
// Fetch dynamic text from Supabase
...
// Read PDF, convert each page to PNG, send to bridge
async function sendToPrinter(pdfPath) {
  // Convert with poppler’s pdftoppm
  const base = pdfPath.replace(/\.pdf$/, "");
  ensureDirectoryExists(base);
  require("child_process").execSync(
    `pdftoppm -r 300 -png "${pdfPath}" "${base}"`
  );
...
async function pollPendingOrders() {
  ...
  for (const order of orders) {
    try {
      ...
      // now print each page as PNG images
