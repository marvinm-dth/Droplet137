...
const elementsConfig = [
  ...
  // ← NEW: vertical solid black line
  {
    ...
    thickness: 2,
    color: "black",
    style: "solid"        // currently solid; can change to "dashed" or "dotted"
  },
];
...
async function renderElements(doc, elements, context, dims) {
  for (const el of elements) {
    ...
    if (el.type === "qr") {
      const qrBuf = await QRCode.toBuffer(content || "UNKNOWN");
      doc.image(qrBuf, x, y, { width: w, height: h });
      continue;
    }
    ...
    if (el.type === "line") {
      ...
      doc.lineWidth(el.thickness || 1)
         ...
         .undash(); // clear any previous dash
