...
const elementsConfig = [
  ...
  },
  {
    ...
    fontColor: "black",
    bounds: { width: 0.55, height: 0.12 },
    position: { x: 0.12, y: 0.90 }
  },
  {
    name: "qr_code",
    type: "qr",
    sourceElement: "sku",
    bounds: { width: 0.40, height: 0.70 },
    position: { x: 0.60, y: 0.00 }
  },
  {
    name: "divider",
    type: "line",
  ...
  for (const el of cfg) {
    ...
    switch (el.type) {
      ...
      case "text":
        ...
        ctx.font      = `${el.fontSize * PX_H}px sans-serif`;
        if (el.rotation) {
          ctx.translate(x, y);
...
// ── 2. Create archival PDF from buffers ─────────────────────────────────────
async function createPdf(bufs, orderId) {
  ...
  bufs.forEach((b, idx) => {
    ...
    doc.image(b, 0, 0);               // let pdfkit size automatically
  });
...
// ── 3. Send PNG buffers to print bridge ─────────────────────────────────────
...
// ── 4. Poll new orders, generate, print, archive ────────────────────────────
