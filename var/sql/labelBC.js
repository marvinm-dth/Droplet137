...
async function insertTrackingRows(order){
  /* Pull only columns that exist in home_depot_items */
  ...
  /* Build one row per physical label */
  const rows = Array.from({length: order.order_qty_requested}, () => ({
    external_sku : externalSku,          // can be NULL
    internal_sku : internalSku,
    item_name    : null,                 // no matching column in source
    item_desc    : item?.item_desc ?? null,
    label_size   : '2x1',
    order_number : order.order_id,
    status       : 'ordered',
    ordered_at   : new Date().toISOString(),
    UUID         : crypto.randomBytes(8).toString('hex')   // 16-char QR-friendly
  }));
