...
// Rasterize PDF → PNG pages, trying pdftoppm first, then Ghostscript
function rasterizePdfToPngs(pdfPath, base) {
  try {
...
// Draw all elements into the PDF page
...
// Create the PDF (2″×1″ @300dpi → 600×300px)
...
// Helper: try pdftoppm, else Ghostscript
function rasterizePdfToPngs(pdfPath, base) {
  try {
    execSync(`pdftoppm -r 300 -png "${pdfPath}" "${base}"`);
  } catch {
    console.warn("pdftoppm failed, using Ghostscript fallback");
    execSync(
      `gs -q -dNOPAUSE -dBATCH -sDEVICE=png16m -r300 -sOutputFile="${base}-%d.png" "${pdfPath}"`
    );
  }
