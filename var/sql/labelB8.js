...
let elementsConfig = [
  ...
  {name:'sku', type:'text', table:'home_depot_items',
   ...
   bounds:{width:0.60,height:0.11}, position:{x:0.69,y:0.7}, wrap:false},
...
/**
 * Greedy word-wrap using only measureText (no raster).
 */
function wrapGreedy(text, measCtx, maxW) {
  ...
  while (words.length) {
    ...
    while (words.length &&
           measCtx.measureText(`${line} ${words[0]}`).width <= maxW) {
      line += ' ' + words.shift();
    }
    out.push(line);
  }
  ...
  for(const el of cfg){
    ...
    switch(el.type){
      case 'text': {
        ...
        // iterative fitter (measure only)
        const meas  = createCanvas(1,1).getContext('2d');
        const mkFont = px => `${el.weight==='700'?'bold ':''}${px}px "Inter"`;
        let fontPx  = el.fontSize * PX_H;
        let lines, lineH, tries = 0;

        do {
          meas.font = mkFont(fontPx);
          lines  = el.wrap ? wrapGreedy(content, meas, w) : [content];
          lineH  = fontPx * 1.2;
          const tooWide = lines.some(l => meas.measureText(l).width > w);
          const tooTall = lines.length * lineH > h;
          if (!tooWide && !tooTall) break;      // fits!
          fontPx -= 0.01 * PX_H;                // shrink ≈2 px
        } while (++tries < 40 && fontPx > 6);

        // draw
        ctx.save();
        ctx.font = mkFont(fontPx);
