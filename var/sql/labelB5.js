...
let elementsConfig = [
  ...
  {name:'item_name', type:'text', table:'home_depot_items',
   ...
   bounds:{width:0.55,height:0.45}, position:{x:0.15,y:0.05}, wrap:true},  // enlarged height, wraps
...
/**
 * Pixel-accurate wrap (char-by-char).
 * Returns an array of lines that each fit within `maxW` pixels.
 */
function wrapLines(text, ctxMeasure, maxW){
  if(!text) return [''];
  const lines=[]; let line=''; let wAcc=0;
  for(const ch of text){
    const chW = ctxMeasure.measureText(ch).width;
    if(wAcc+chW>maxW && line){
      lines.push(line.trimEnd()); line=''; wAcc=0;
    }
    line+=ch; wAcc+=chW;
  }
...
async function drawElements(ctx, cfg, ctxObj){
  ...
  for(const el of cfg){
    ...
    switch(el.type){
      // ─── replace only the text-case inside drawElements ───────────────────────
case 'text': {
  if (el.fillColor) { ctx.fillStyle = el.fillColor; ctx.fillRect(x, y, w, h); }

  ctx.save();

  // --- build a function that (re)creates font, measurer & wrapped lines ---
  function shape(fontPx) {
    const fspec = `${el.weight === '700' ? 'bold ' : ''}${fontPx}px "Inter"`;
    const meas  = createCanvas(1, 1).getContext('2d');
    meas.font   = fspec;
    return {
      spec: fspec,
      px:   fontPx,
      lines: el.wrap ? wrapLines(content, meas, w) : [content],
      meas
    };
  }

  // first attempt at declared size
  let shaped = shape(el.fontSize * PX_H);
  const lineH = () => shaped.px * 1.2;

  // --- shrink loop if overdraw persists -----------------------------------
  let tries = 0;
  while (tries < 40) {
    const tooWide  = shaped.lines.some(l => shaped.meas.measureText(l).width > w);
    const tooTall  = shaped.lines.length * lineH() > h;
    if (!tooWide && !tooTall) break;      // fits!
    shaped = shape(shaped.px - 0.01 * PX_H);   // shrink 1 % of label height
    tries++;
  }

  // --- draw ----------------------------------------------------------------
  ctx.font = shaped.spec;
  ...
  for (let i = 0; i < shaped.lines.length; i++) {
    const ly = y + i * lineH();
    if (i > 0 && ly + lineH() > y + h) break;   // vertical guard
    ctx.fillText(shaped.lines[i], x, ly);
  }
  ctx.restore();
} break;
...
async function createPdf(bufs,id){
  ...
  doc.end(); await new Promise(r=>st.on('finish',r)); return fp;
}
