...
[${processed}/${items.length}] Processing material_id=${item.material_id}`);
    ...
    const systemPrompt =
      `You are a helpful assistant that creates concise, human-readable short names for item descriptions.` +
      ` Each short name must be no longer than ${maxLen} characters. Return ONLY the short name.`;
