...
const masterElements = [
  ...
  /* qr_code text supplied per-label (sku+uuid) */
...
/* Create one PNG; optionally insert tracking row */
async function buildLabel({order, internalSku, externalSku, itemDesc, preview=false}){
  ...
  /* insert tracking row only for real print */
  if(!preview){
    const row = {
...
/* Build preview buffers (no DB writes) */
...
/* Package PDF (95 % scale, anchored LR) */
