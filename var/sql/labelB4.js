  ...
  for(const el of cfg){
    ...
    switch(el.type){
      case 'text': {
        ...
        ctx.save();
        const px = el.fontSize*PX_H;
        const fontSpec = `${el.weight==='700'?'bold ':''}${px}px "Inter"`;
        ctx.font = fontSpec;
        ...
        // off-screen measurer uses identical font
        const measurer = createCanvas(1,1).getContext('2d');
        measurer.font = fontSpec;

        const lines = el.wrap ? wrapLines(content, measurer, w) : [content];
        const lh = px*1.2;
        for(let i=0;i<lines.length;i++){
          const ly=y+i*lh;
          if(i>0 && ly+lh>y+h) break;      // guard vertical overflow
