...
async function pollPendingOrders(){
  ...
  for(const order of orders){
    try{
      ...
      // keep a copy for the preview endpoints
      lastOrder   = order;
      lastBuffers = bufs;
      lastPdfPath = pdfPath;
...
// ════════════════════════════════════════════════════════════════════════════
// ↓↓↓↓↓         NEW  R E S T   A P I  𝘧𝘰𝘳  preview & live editing        ↓↓↓↓↓
// ════════════════════════════════════════════════════════════════════════════

// --- read template ---
app.get('/elements-config', (_,res)=> res.json(elementsConfig));

// --- replace entire template (PUT) or update one element by name (PATCH) ---
...
// --- expose last order data so the designer can pre-fill forms ---
app.get('/last-order', (_,res)=> lastOrder ? res.json(lastOrder) : res.status(404).json({error:'No order rendered yet'}));

// --- manual preview : POST {order_id, order_qty_requested, material_id} ---
...
// --- fetch the N-th PNG of the last render ---
...
// --- download the last preview PDF ---
app.get('/preview/pdf', (req,res)=>{
  if(!lastPdfPath||!fs.existsSync(lastPdfPath)) return res.status(404).send('No PDF available');
