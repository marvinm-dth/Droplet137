...
router.get('/status/:status', async (req, res) => {
    try {
        ...
        const data = await ProjectModel.getByFilter({ status });
        res.status(200).json(data);
    } catch (error) {
        console.log(error);
        res.status(500).json({ error: `Server error: ${error}` });
...
router.post('/new', async (req, res) => {
    ...
    try {
        const checklist_data = await ProjectModel.insertOne({ project_id, checklist_template_id });
...
router.post('/:id/update', async (req, res) => {
    try {
        ...
        const data = await ProjectModel.update(id, updateData)
