...
HOMEDEPOT_SPIDER = os.path.abspath(os.path.join(os.path.dirname(__file__), 'homedepot_spider'))
...
UPLOAD_FOLDER_LOCATION = '/var/sql/dth_materials/location'
UPLOAD_FOLDER_BARCODE_LOC = '/var/sql/dth_materials/barcode_loc'
...
app.config['UPLOAD_FOLDER_BARCODE_LOC'] = UPLOAD_FOLDER_BARCODE_LOC
...
def build_location_tree(rows):
    ...
    for row in rows:
        parent_barcode = row.get('parent_barcode')
        if parent_barcode and parent_barcode in lookup:
            lookup[parent_barcode]['children'].append(row)
        else:
            roots.append(row)
...
# Route for serving uploaded files
@app.route('/inventory_loc/<filename>')
def uploaded_location_item(filename):
    return send_from_directory(app.config['UPLOAD_FOLDER_LOCATION'], filename)

# Route for serving uploaded files
@app.route('/barcode_loc/<filename>')
def uploaded_barcode_loc_item(filename):
    return send_from_directory(app.config['UPLOAD_FOLDER_BARCODE_LOC'], filename)
    
import os, base64

def save_base64_to_file(base64_data, folder, filename, default_ext='png'):
    """
    Saves a base64-encoded image (with or without Data-URI header) to disk.

    Args:
      base64_data (str): Either "data:<mime>;base64,<data>" or just "<data>".
      folder (str): Directory to save into.
      filename (str): Base filename (without extension).
      default_ext (str): Extension to use if no header was provided.

    Returns:
      str: The basename of the saved file (e.g. "foo.png").
    """
    # If there's a header, split it out; otherwise assume raw base64
    if ',' in base64_data:
    ...
    # Build a *relative* filename and full disk path
    file_on_disk = f"{filename}.{ext}"
    full_path    = os.path.join(folder, file_on_disk)

    # Write out the decoded bytes
    with open(full_path, 'wb') as f:
        f.write(base64.b64decode(encoded))

    # **Return only the basename** so callers can build URLs cleanly:
    return file_on_disk
        ...
        for supplier in suppliers:
            ...
            for id, name in zip(data['approver_ids'], data['approver_names']):
                notif_data = {'category': "order",
                            "url": f"https://inventory.orcagroup.io:5023/approval_order/{order_insert.data[0]['order_id']}",
...
def submit_approval_order(order_id):
    ...
    if approval_status == "Approved":
        ...
        db.insert("dth_notification", {'url': f"https://inventory.orcagroup.io:5023/approval_order/{order_id}",
...
def submit_supplier():
    ...
    while f"materials[{index}][material_id]" in request.form:
        ...
        db.insert("home_depot_items", {"internet_sku_number": supplier_material_id,
                                       ...
                                       "item_image": f"https://inventory.orcagroup.io:5023/image/{str(supplier_name)}_{str(supplier_id)}.jpg"
                                       })
...
def fetch_item_details():
    ...
    if supplier_name == "Home Depot":
        ...
        project_root   = os.path.join(repo_root, 'homedepot_spider')        # /var/sql/amazon_spider
        ...
        cmd = [
            'scrapy', 'crawl', 'dynamic_hd',                               # spider name = dynamic
...
def add_material(supplier_id):
    if current_user.role == "user":
        ...
        response_loc = db.fetch_all('dth_location')
        if response:
            supplier_data = response.data[0]
            inventory_loc = response_loc.data
            return render_template("add_material.html", supplier_data=supplier_data, inventory_loc = inventory_loc)
...
def submit_material(supplier_id):
    ...
    if response:
        ...
        response_loc = db.fetch_one('dth_location', id, inv_location)
        res_loc = response_loc[0]['display_name']
        ...
        db.insert("home_depot_items", {"internet_sku_number": supplier_sku_number,
                                       ...
                                       "inventory_location": res_loc,
                                       "inventory_location_id": inv_location,
                                       "keywords": keywords,
                                       "reorder_point": int(reorder_point),
                                       "pack_size": int(pack_size),
                                       "item_price": item_price,
                                       "supplier_id": supplier_id,
                                       "item_image": f"https://inventory.orcagroup.io:5023/image/{str(supplier_name)}_{str(supplier_sku_number)}.jpg"
                                       })
        ...
        for file in new_image:
            if file:
                ...
                db.insert("dth_delivery_images", {"delivery_id": delivery_data['delivery_id'],
                                                "image_url":f"https://inventory.orcagroup.io:5023/delivery_file/{filename}"})
                
        db.insert("dth_notification", {"url": f"https://inventory.orcagroup.io:5023/approval_delivery/{delivery_data['delivery_id']}",
                                       "requester_name": current_user.name,
                                       'requester_id': current_user.id,
                                       "approval_status": "pending",
                                       "approver_name": approver_name,
...
def onhand_inventory_used_bar():
    ...
    new_inv_data = db.update("home_depot_items", "material_id", material_id, {"inventory": new_inventory,
                                                                              "latest_manual_log_date": datetime.now().strftime('%Y-%m-%d %H:%M:%S')})
...
@app.route("/inventory_location", methods=["GET", "POST"])
def inventory_location():
    dth_loc = db.fetch_all("dth_location")
    dth_loc_data = dth_loc.data
    dth_loc_data = build_location_tree(dth_loc_data)
    print(dth_loc_data)
    return render_template("inventory_loc.html", user=current_user, locations=dth_loc_data)
...
def save_locations():
    ...
    loc_map = { loc['barcode']: loc for loc in locations }

    for loc in locations:
        print(loc.get('image_base64'))
        barcode_val = loc['barcode']
        parent = loc.get('parent_barcode')
        
        # 1) Save images to disk
        image_url = ''
        if loc.get('image_base64'):
            image_name = save_base64_to_file(
                loc['image_base64'],
                app.config['UPLOAD_FOLDER_LOCATION'],    # e.g. "static/inventory_loc"
                f"{barcode_val}_image"
            )
        elif loc.get("image_url"):
            # just use that filename directly:
            image_name = loc["image_url"]
        barcode_url = ''
        if loc.get('barcode_base64'):
            barcode_url = save_base64_to_file(
                loc['barcode_base64'],
                app.config['UPLOAD_FOLDER_BARCODE_LOC'], # e.g. "static/barcode_loc"
                f"{barcode_val}_barcode"
            )

        # start with this loc’s own name
        display_parts = [loc['name']]
        current = parent
        # as long as the parent is in this payload, prepend its name
        while current and current in loc_map:
            parent_loc = loc_map[current]
            display_parts.insert(0, parent_loc['name'])
            current = parent_loc.get('parent_barcode')
        ...
        record = {
            ...
            "notes":          loc["notes"],
            "barcode":        barcode_val,
            "image_url":      f"https://inventory.orcagroup.io:5023/inventory_loc/{image_name}",
            "barcode_url":    barcode_url,
            "parent_barcode": parent,
            "display_name": display_name
            # if you track ordering:
            # "position": loc.get("position", 0),
        }

        # 3) Check existence
        existing = db.fetch_one("dth_location", "barcode", barcode_val).data
        if existing:
            # update the existing row
            db.update("dth_location", "barcode", barcode_val, record)
        else:
            # insert a new one
            db.insert("dth_location", record)
...
def get_home_depot_items():
    ...
    for item in response.data:
        ...
        # Add supplier_name to the item dictionary
        item['supplier_name'] = supplier_name
        item['qr_info'] = f"https://inventory.orcagroup.io:5023/home_depot_item/{str(item['material_id'])}"

        # CHANGE THIS
        item['qr_inventory'] = f"https://inventory.orcagroup.io:5023/inventory/{str(item['material_id'])}"
...
def get_home_depot_item(material_id):
    ...
    if response.data:
        ...
        raw = item.get("upc", "")                # TEXT containing a Python list
        try:
            lst = ast.literal_eval(raw)               # now a Python list
            if isinstance(lst, list):
                # join into one string, e.g. "cut-to-size, weatherproof, UV-resistant"
                item["upc"] = ", ".join(str(x) for x in lst)
            else:
                # if it wasn’t actually a list, leave it alone
                pass
        except (ValueError, SyntaxError):
            # if parsing fails, you can fallback to a simpler split:
            parts = [p.strip().strip("'\"") for p in raw.strip("[]").split(",") if p]
            item["upc"] = ", ".join(parts)
...
def get_home_depot_delivery(order_id):
    ...
    if response_order.data:
        ...
        return render_template("delivery.html", order_id=order_id,order_data=order_data, order_delivery_data=order_delivery_data , user=current_user)
    return jsonify({"message": "Checklist item not found"}), 404
...
def update_order_form(order_id):
    ...
    if (receipt_file) and (f"https://inventory.orcagroup.io:5023/receipt/receipt_{str(order_id)}.pdf" != order_data_old['supplier_receipt']):
        ...
        db.update("home_depot_orders", "order_id", order_id, {"supplier_receipt": f"https://inventory.orcagroup.io:5023/receipt/{receipt_filename}"})
    else:
        print("NO")
    ...
    if (qty_file) and (f"https://inventory.orcagroup.io:5023/order_file/order_{str(order_id)}.pdf" != order_data_old['supplier_order_file']):
        qty_filename = f"order_{order_id}.pdf"
        qty_file.save(os.path.join(app.config['UPLOAD_FOLDER_ORDER_FILE'], qty_filename))
        db.update("home_depot_orders", "order_id", order_id, {"supplier_order_file": f"https://inventory.orcagroup.io:5023/order_file/{qty_filename}"})
    ...
    if other_files:
        for file in other_files:
            ...
            other_file_dict = {"order_id": order_id,
                               "file_name": file_name,
                               "file_url": f"https://inventory.orcagroup.io:5023/order_file/{file_name}"}
...
def update_material(material_id):
    response =  db.fetch_one("home_depot_items", "material_id", material_id)
    inventory_loc_res = db.fetch_all("dth_location")
    if response:
        item_data = response.data[0]
        inventory_loc = inventory_loc_res.data
        print(inventory_loc)
        unescaped_upc = html.unescape(item_data['upc'])
        print(unescaped_upc)
        item_data['upc'] = ast.literal_eval(unescaped_upc)
        return render_template("update_material.html", item = item_data, inventory_loc = inventory_loc, user=current_user)
...
def submit_update_material(material_id):
    ...
    old_inventory_location_id = material_old_data['inventory_location_id']
    ...
    new_inventory_location = request.form.get("inv_location_display_name")
    new_inventory_location_id = request.form.get('inv_location')
    print(new_inventory_location_id)
    ...
    if str(old_inventory_location) != str(new_inventory_location_id):
        db.update("home_depot_items", "material_id", material_id, {"inventory_location_id": new_inventory_location_id})
        response_inv = db.fetch_one("dth_location", "id", new_inventory_location_id)
        res_data = response_inv.data[0]
        inventory_location = res_data['display_name']
        db.update("home_depot_items", "material_id", material_id, {"inventory_location": inventory_location})
    ...
    if new_label_size and new_label_size != material_old_data.get("label_size"):
        db.update("home_depot_items", "material_id", material_id,
    ...
    if new_image:
        ...
        db.update("home_depot_items", "material_id", material_id, {"item_image": f"https://inventory.orcagroup.io:5023/image/{str(supplier_name)}_{str(new_supplier_sku)}.jpg"})
    ...
    for file in uploaded_files:
        if file:
            ...
            db.insert("dth_material_images", {"material_id": material_id,
                                              "image_url":f"https://inventory.orcagroup.io:5023/image/{filename}"})
...
def add_user_order():
    ...
    new_id = db.insert("dth_user_order", to_insert_data)  # returns the generated PK
    print(new_id.data[0])
    return jsonify({
      'success': True,
      'id': new_id.data[0]["id"]
...
def send_receive_material():
    ...
    for image in global_images:
        ...
        to_insert_img_data = {"receive_id" : receive_data.data[0]['id'],
                              "image": f"https://inventory.orcagroup.io:5023/receive/{filename}"}
    ...
    for material in receive_materials:
        ...
        for receive_image in receive_materials_images:
            ...
            to_insert_material_img = {"receive_item_id": receive_material_data.data[0]['id'],
                                      "image": f"https://inventory.orcagroup.io:5023/receive_item/{filename}"}
...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=5023, ssl_context=('/etc/letsencrypt/live/inventory.orcagroup.io/fullchain.pem', '/etc/letsencrypt/live/inventory.orcagroup.io/privkey.pem'))